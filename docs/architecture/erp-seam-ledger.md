...
| 8 | `backend/schedulers.py` + `backend/clearing_settlement_scheduler.py` + `backend/run_schedulers.py` + `backend/models.py` + `docker-compose.local.yml` | 2026-07-28 | S1–S5 scheduler hardening. S1: `_SCHEDULER_STARTERS` dict + `_CRITICAL` frozenset → CRITICAL failure before `run_forever()` → `sys.exit(1)`; OPTIONAL failures → WARNING only. S2: loop exception → `_failed.set()` + `os.kill(SIGTERM)` (graceful-first); join timeout → `os._exit(1)` (sole forced-exit site). S3: `_stop_event.wait(timeout=60)` replaces `time.sleep(60)` — `stop()` wakes sleeping thread in < 1 s. S4: `_thread` ref stored on scheduler; `stop()/join(30)` sequence in `main()`; `stop_grace_period: 35s` in Docker. S5: `ReconciliationFinding` model (`reconciliation_findings` table) + `_emit_stale_finding_if_needed()` writes `STALE_SETTLEMENT` kind when no `auto_settlement` voucher within threshold; `check_count` incremented on re-check. Architectural gaps recorded as SCHED-001/002/003 in `docs/architecture/architecture-v1.md §4.6`. | ✅ 10 tests in `backend/test_scheduler_hardening.py` (S1×3, S2a×1, S2b×1, S3×1, S4×2, S5×2) + 11 existing tests in `test_scheduler_architecture.py`; full suite: **21 passed, 0 failed** | Sprint 9 ERP Hardening |
## Pending Rows (to be filled when the work is done)

| # | Expected Touch | Sprint | Who |
|---|---------------|--------|-----|
| 9 | (next integration touch) | — | — |
...
| 2026-07-28 | 8 | 8 | 100% | S1–S5 scheduler hardening + SCHED-001/002/003 Known Gaps |
