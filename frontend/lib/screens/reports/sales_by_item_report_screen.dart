...
class SalesByItemReportScreen extends StatefulWidget {
...
class _SalesByItemReportScreenState extends State<SalesByItemReportScreen> {
  ...
  Future<void> _loadReport() async {
    ...
    try {
      final result = await widget.api.getSalesByItemReport(
  ...
  String _localizedOrderLabel(String key) {
    ...
    switch (key) {
      ...
      case 'sales_value':
        return isArabic ? 'إجمالي المبيعات' : 'Sales';
      case 'returns_value':
        return isArabic ? 'المرتجعات' : 'Returns';
      case 'net_weight':
        return isArabic ? 'صافي الوزن' : 'Net Weight';
      case 'sales_weight':
        return isArabic ? 'وزن المبيعات' : 'Sales Weight';
      case 'returns_weight':
        return isArabic ? 'وزن المرتجعات' : 'Returns Weight';
      case 'net_quantity':
        return isArabic ? 'صافي الكمية' : 'Net Quantity';
      case 'sales_quantity':
        return isArabic ? 'كمية المبيعات' : 'Sales Quantity';
      case 'returns_quantity':
        return isArabic ? 'كمية المرتجعات' : 'Returns Quantity';
      case 'documents':
        return isArabic ? 'المستندات' : 'Documents';
      case 'average_price_per_gram':
        return isArabic ? 'متوسط السعر/جرام' : 'Avg Price/g';
  ...
  Widget build(BuildContext context) {
    ...
    return Directionality(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: Text(isArabic ? 'تقرير المبيعات حسب الصنف' : 'Sales by Item'),
          actions: [
            IconButton(
  ...
  Widget _buildSummaryCard(bool isArabic) {
    ...
    final metrics = [
      ...
      _SummaryMetric(
        label: isArabic ? 'عدد المستندات' : 'Documents',
      ...
      _SummaryMetric(
        label: isArabic ? 'متوسط السعر/جرام' : 'Avg Price/Gram',
        value: _formatCurrency(_asDouble(summary['average_price_per_gram'])),
  ...
  Widget _buildChartCard(bool isArabic) {
    final items = List<Map<String, dynamic>>.from(
      _report?['items'] as List<dynamic>? ?? const [],
    );
    ...
    final hasValues = items.any((row) => _asDouble(row['net_value']).abs() > 0);
  ...
  Widget _buildTableCard(bool isArabic) {
    ...
    if (items.isEmpty) {
      return _buildEmptyState(
        ...
        message: isArabic ? 'لا توجد بيانات أصناف.' : 'No item data available.',
      );
    }
            ...
            SingleChildScrollView(
              ...
              child: DataTable(
                ...
                rows: items.map((item) {
                  ...
                  final formattedDate = lastDate == null || lastDate == ''
                      ? '-'
                      : dateFormat.format(DateTime.parse(lastDate));
                  final itemLabel = item['item_code'] != null
...
class _TopItemsChart extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final data = items.asMap().entries.map((entry) {
      ...
      final label = row['item_code'] != null
          ? '${row['item_name']} (${row['item_code']})'
          : row['item_name']?.toString() ?? '';
      return _ChartPoint(
        index: index,
        label: label,
        value: _asDouble(row['net_value']).abs(),
      );
    }).toList();
