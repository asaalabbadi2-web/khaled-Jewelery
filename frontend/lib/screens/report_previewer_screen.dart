...
class _ReportPreviewerScreenState extends State<ReportPreviewerScreen> {
        ...
        children: filters
            .map(
              (filter) => pw.Padding(
                ...
                child: pw.Text(filter, style: const pw.TextStyle(fontSize: 10)),
              ),
            )
            .toList(),
      ),
  ...
  void _exportPdf() {
    Printing.layoutPdf(
      onLayout: (_) => _export(),
      name:
          '${widget.reportType}_${DateFormat('yyyyMMdd_HHmm').format(DateTime.now())}.pdf',
    );
  }
    ...
    return Scaffold(
      ...
      body: PdfPreview(
        ...
        onError: (context, error) {
          ...
          return Center(
            child: Text(
              widget.isArabic ? 'تعذر إنشاء ملف PDF' : 'Failed to generate PDF',
            ),
          );
        },
      ),
