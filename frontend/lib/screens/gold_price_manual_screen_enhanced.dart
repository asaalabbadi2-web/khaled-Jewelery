...
class _GoldPriceManualScreenEnhancedState
  ...
  Future<void> _updatePrice() async {
    if (!_useUsdInput && _priceController.text.isEmpty) {
      _showMessage('يرجى إدخال سعر الغرام بالريال أو التبديل إلى الدولار/أونصة', isError: true);
      return;
    }
    if (_useUsdInput && _priceUsdController.text.isEmpty) {
      _showMessage('يرجى إدخال سعر الأونصة بالدولار أو إلغاء التبديل', isError: true);
      return;
    }
    ...
    // تأكيد التحديث
    final confirm = await _showConfirmDialog(_useUsdInput ? _usdToSar(usdValue!) : sarValue!);
    if (!confirm) return;
    ...
    try {
      ...
      final payload = _useUsdInput
          ? {
              'price_usd_per_oz': usdValue,
            }
          : {
              'price': sarValue,
              'unit': 'sar_per_gram',
    ...
    final fallbackValue = _useUsdInput ? _usdToSar(usdValue!) : sarValue!;
    final updatedSar = decoded['price_sar_per_gram'] ?? decoded['price_24k'] ?? fallbackValue;
    final updatedPrice = updatedSar is num ? updatedSar.toDouble() : double.tryParse('$updatedSar') ?? fallbackValue;
