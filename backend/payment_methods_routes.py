...
def update_payment_method(id):
    ...
    try:
        ...
        if 'payment_type' in data:
            ...
            parent_account_id = payment_method.account.parent_id if payment_method.account else None

            if parent_account_id:
                duplicate_for_update = (
                    PaymentMethod.query
                    .join(Account, PaymentMethod.account_id == Account.id)
                    .filter(
        ...
        if 'name' in data:
            ...
            if payment_method.account:
                payment_method.account.name = data['name']
