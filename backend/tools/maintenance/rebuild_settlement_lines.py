...
قاعدة التاريخ:
  كل سند يرى فقط IPs بـ created_at ≤ voucher.date + 3 أيام.
  يمنع "السرقة" عبر الزمن.

نتيجة متوقعة:
  كل سند مغطّى بالكامل (SL == amount_cash) ✅
  استثناء AV-2026-00133: فجوة 6,050 SAR موثّقة ومقبولة.
    - السبب: IP#1547 (جزء) + IP#15xx داخل النافذة لكنها
      استُهلكت بـ AV-2026-00236 (3,930) + AV-2026-00237 (2,120).
    - المال موجود والـ IPs مغطاة — الفجوة في نسب التوزيع فقط.
    - لا يلزم قيد محاسبي.
